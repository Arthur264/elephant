...
var AppService = (function () {
    function AppService(http) {
        this.http = http;
        this.hostname = 'http://elephant-artyr264.c9users.io:8081/api/v1/';
    ...
    AppService.prototype.get = function (url, params) {
        ...
        params['format'] = 'json';
        var headers = new http_1.Headers({ 'Authorization': 'Token ' + this.token });
        var options = new http_1.RequestOptions({ params: params, headers: headers });
    ...
    AppService.prototype.delete = function (url) {
        var headers = new http_1.Headers({ 'Content-Type': 'application/json', 'Authorization': 'Token ' + this.token });
        var options = new http_1.RequestOptions({ headers: headers });
    ...
    AppService.prototype.upload = function (url, data) {
        var headers = new http_1.Headers({ 'Authorization': 'Token ' + this.token, 'Accept': 'application/json' });
        var options = new http_1.RequestOptions({ headers: headers });
        return this.http.post(this.hostname + url, data, options)
            .map(function (res) { return res.json(); });
    };
